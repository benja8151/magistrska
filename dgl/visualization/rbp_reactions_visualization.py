...
def testModelOnGraph(model, testGraph, isRBP, testPrediction=False):
    ps = model(testGraph, testGraph.edata['feat'], testGraph.ndata['feat'])
    prob = torch.sigmoid(ps)
    predictionRBP = torch.round(prob).item()

    if testPrediction and ((isRBP and predictionRBP != 1) or (not isRBP and predictionRBP != 0)):
        print(ps)    
        raise Exception("Model did not predict correctly!")
    
    if (isRBP):
        return prob.item() # prob should be closer to 1
    else:
        return 1-prob.item()  # prob should be closer to 0
...
def evaluateModel(
    ...
    originalProb = testModelOnGraph(model, testGraph, isRBP, True)
    ...
    for compound, mapping in atomMappings.items():
        ...
        if len(mapping) == 2:
            ...
            removedProbs.append(testModelOnGraph(model, modifiedGraph, isRBP))
        # Larger compounds
        else:
    ...
    fig = plt.figure()
    ax = fig.add_axes([0.05, 0.80, 0.7, 0.08])
    ...
    doc.save(outputDir + '/' + reactionName + '.svg')
...
"""
evaluateModel(
    modelPath='C:/Users/Benjamin/Documents/Datoteke_za_solo/MAG/magistrska/dgl/model/rbpmodel.pt',
    ...
    outputDir='C:/Users/Benjamin/Documents/Datoteke_za_solo/MAG/magistrska/dgl/visualization/results/RBPReactions'
)
