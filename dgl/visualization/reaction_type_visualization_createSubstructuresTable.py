...
def evaluateModel(
    ...
    for compound, mapping in atomMappings.items():
        ...
        if len(mapping) == 2:
            ...
            removedDeltaValue = originalProb - removedProb if useAbsoluteDeltas else (originalProb -  removedProb) / originalProb
...
class StructureDeltaResult: 
    ...
    def calculateMaxMin(self, reactionType: int):
        if self.averageDelta > 0:
            maxMinDeltas[reactionType]["max"] = min(maxMinDeltas[reactionType]["max"], self.averageDelta)
        else:
...
def compareReactionsBySubstructures(
            ...
            for molKey in filteredStructures.keys():
                if (molKey.HasSubstructMatch(structureMol) and structureMol.HasSubstructMatch(molKey)):
                    ...
                    if reactionType in filteredStructures[molKey]:
                        filteredStructures[molKey][reactionType]["neg"] = StructureDeltaResult(structureDeltas, reactionType)
                    else:
                        filteredStructures[molKey][reactionType] = {"neg": StructureDeltaResult(structureDeltas, reactionType)}
                    structExists = True
                    break
            if not structExists:
                filteredStructures[structureMol] = {reactionType: {"neg": StructureDeltaResult(structureDeltas, reactionType)}}
        ...
        while structuresCount < min(5, len(typeStructures)):
            ...
            for molKey in filteredStructures.keys():
                if (molKey.HasSubstructMatch(structureMol) and structureMol.HasSubstructMatch(molKey)):
                    ...
                    # Check for structure doubling for each reaction type
                    if reactionType in filteredStructures[molKey]:
                        filteredStructures[molKey][reactionType]["pos"] = StructureDeltaResult(structureDeltas, reactionType)
                    else:
                        filteredStructures[molKey][reactionType] = {"pos": StructureDeltaResult(structureDeltas, reactionType)}
                    structExists = True
                    break
            if not structExists:
                filteredStructures[structureMol] = {reactionType: {"pos": StructureDeltaResult(structureDeltas, reactionType)}}
    ...
    worksheet1.set_column_pixels("B:B", 160, cell_format=workbook.add_format({
        'bold': True, 
        'align': 'center', 
        'valign': 'vcenter',
        'right': 2
     }))
    worksheet2.set_column_pixels("B:B", 160, cell_format=workbook.add_format({
    'bold': True, 
    'align': 'center', 
    'valign': 'vcenter',
    'right': 2
    }))
    worksheet1.set_row_pixels(0, 26, cell_format=workbook.add_format({
        'bold': True, 
        'align': 'center', 
        'valign': 'vcenter',
        'bottom': 2
    }))
    worksheet2.set_row_pixels(0, 26, cell_format=workbook.add_format({
        'bold': True, 
        'align': 'center', 
        'valign': 'vcenter',
        'bottom': 2
    ...
    for mol in filteredStructures.keys():
        ...
        worksheet1.set_row_pixels(mol_id + 1, rowHeight, cell_format=workbook.add_format({
            'bottom': 3,
            'align': 'center', 
            'valign': 'vcenter',
        }))
        worksheet2.set_row_pixels(mol_id + 1, rowHeight, cell_format=workbook.add_format({
            'bottom': 3,
            'align': 'center', 
            'valign': 'vcenter',
        }))
        worksheet1.insert_image('A' + str(mol_id + 2), imagePath, {'x_offset': (maxImageWidth- imageWidth)/2, 'y_offset': int((rowHeight-imageHeight)/2)})
        worksheet2.insert_image('A' + str(mol_id + 2), imagePath, {'x_offset': (maxImageWidth- imageWidth)/2, 'y_offset': int((rowHeight-imageHeight)/2)})
        ...
        for reactionType in evaluatedReactionTypes:
            ...
            resultColors2 = []
            if "pos" in result:
                resultStrings.append(f"+{round(result['pos'].averageDelta, 3)} ({result['pos'].count} occurence{'s' if result['pos'].count > 1 else ''})")
            ...
            if "neg" in result:
                if len(resultStrings) == 0:
                    resultStrings.append(f"{round(result['neg'].averageDelta, 3)} ({result['neg'].count} occurence{'s' if result['neg'].count > 1 else ''})")
...
compareReactionsBySubstructures(
    ...
    outputName="structureComparisonTable2",
